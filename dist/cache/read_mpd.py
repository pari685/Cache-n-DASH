...
def get_tag_name(xml_element):
    ...
    try:
        ...
        config_cdash.LOG.error("Unable to retrieve the tag. ")
        return None
    return tag_name
...
def read_mpd(mpd_file):
    ...
    config_cdash.LOG.info("Reading the MPD file")
    try:
            ...
            for adaptation_set in element:
                for representation in adaptation_set:
                    if "SegmentTemplate" in get_tag_name(representation.tag):
                                ...
                                config_cdash.LOG.info('base_url = {}'.format(representation.attrib['media']))
                                dash_playback_object.video['start'] = int(representation.attrib['startNumber'])
                                config_cdash.LOG.info('start = {}'.format(representation.attrib['startNumber']))
                                dash_playback_object.video['timescale'] = float(representation.attrib['timescale'])
                                config_cdash.LOG.info('timescale = {}'.format(representation.attrib['timescale']))
                                dash_playback_object.video['initialization'] = representation.attrib['initialization']
                                config_cdash.LOG.info('initialization = {}'.format(representation.attrib['initialization']))
                                dash_playback_object.video['duration'] = int(representation.attrib['duration'])
                                config_cdash.LOG.info('duration  = {}'.format(representation.attrib['duration']))
    ...
    if len(dash_playback_object.video['bandwidth_list']) > 0:
        #config_cdash.JSON_HANDLE['playback_info']['available_bitrates'] = dash_playback_object.video['bandwidth_list']
        bitrates_str = [str(i) for i in dash_playback_object.video['bandwidth_list']]
        config_cdash.LOG.info('The video has the following bitrates: {}'.format(','.join(bitrates_str)))
