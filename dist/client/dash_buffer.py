...
class DashPlayer:
    ...
    def __init__(self, video_length, segment_duration):
        config_client.LOG.info("Initializing the Buffer")
        ...
        # Buffer size
        if config_client.MAX_BUFFER_SIZE:
            self.max_buffer_size = config_client.MAX_BUFFER_SIZE
        else:
        ...
        # Buffer Constants
        self.initial_buffer = config_client.INITIAL_BUFFERING_COUNT
        #self.alpha = config_client.ALPHA_BUFFER_COUNT
        #self.beta = config_client.BETA_BUFFER_COUNT
        self.segment_limit = None
        # Current video buffer that holds the segment data
        self.buffer = Queue.Queue()
        self.buffer_lock = threading.Lock()
        self.current_segment = None
        self.buffer_log_file = config_client.BUFFER_LOG_FILENAME
        config_client.LOG.info("VideoLength={},segmentDuration={},MaxBufferSize={},InitialBuffer(secs)={}".format(self.playback_duration,
    ...
    def set_state(self, state):
        ...
        if state in PLAYER_STATES:
            ...
            config_client.LOG.info("Changing state from {} to {} at {} Playback time ".format(self.playback_state, state,
                                                                                            self.playback_timer.time()))
            self.playback_state = state
            self.playback_state_lock.release()
        else:
            config_client.LOG.error("Unidentified state: {}".format(state))
    ...
    def initialize_player(self):
        ...
        config_client.LOG.info("Initialized player with video length {}".format(self.playback_duration))
        while True:
            # Video stopped by the user
            if self.playback_state == "END":
                config_client.LOG.info("Finished playback of the video: {} seconds of video played for {} seconds".format(
            ...
            if self.playback_state == "STOP":
                ...
                config_client.LOG.info("Player Stopped at time {}".format(
            ...
            if self.playback_state == "PAUSE":
                if not paused:
                    ...
                    config_client.LOG.info("Player Paused after {:4.2f} seconds of playback".format(
                        self.playback_timer.time()))
                    self.playback_timer.pause()
                    paused = True
                continue
            ...
            if self.playback_state == "BUFFERING":
                if not buffering:
                    config_client.LOG.info("Entering buffering stage after {} seconds of playback".format(
                        self.playback_timer.time()))
                    self.playback_timer.pause()
                    buffering = True
                    interruption_start = time.time()
                    config_client.JSON_HANDLE['playback_info']['interruptions']['count'] += 1
                # If the size of the buffer is greater than the RE_BUFFERING_DURATION then start playback
                    ...
                    if ((self.buffer.qsize() >= config_client.RE_BUFFERING_COUNT) or (
                            config_client.RE_BUFFERING_COUNT * self.segment_duration >= remaining_playback_time
                        ...
                        if interruption_start:
                            ...
                            interruption_start = None
                            config_client.JSON_HANDLE['playback_info']['interruptions']['events'].append(
                                (interruption_start, interruption_end))
                            config_client.JSON_HANDLE['playback_info']['interruptions']['total_duration'] += interruption
                            config_client.LOG.info("Duration of interruption = {}".format(interruption))
                        self.set_state("PLAY")
                        self.log_entry("Buffering-Play")

            if self.playback_state == "INITIAL_BUFFERING":
                if self.buffer.qsize() < config_client.INITIAL_BUFFERING_COUNT:
                    initial_wait = time.time() - start_time
                    continue
                else:
                    config_client.LOG.info("Initial Waiting Time = {}".format(initial_wait))
                    self.set_state("PLAY")
            ...
            if self.playback_state == "PLAY":
                    ...
                    if self.buffer.qsize() == 0:
                        config_client.LOG.info("Buffer empty after {} seconds of playback".format(
                            self.playback_timer.time()))
                        self.playback_timer.pause()
                    ...
                    config_client.LOG.info("Reading the segment number {} from the buffer at playtime {}".format(
                    ...
                    while self.playback_timer.time() < future:
                        ...
                        if not self.playback_start_time:
                            ...
                            config_client.LOG.info("Started playing with representation {} at {}".format(
                                play_segment['bitrate'], self.playback_timer.time()))

                        # Duration for which the video was played in seconds (integer)
                        if self.playback_timer.time() >= self.playback_duration:
                            config_client.LOG.info("Completed the video playback: {} seconds".format(
                        ...
                        config_client.LOG.debug("Decrementing buffer_length by {}. dash_buffer = {}".format(
                    ...
                    if self.segment_limit:
                        if int(play_segment['segment_number']) >= self.segment_limit:
                            self.set_state("STOP")
                            config_client.LOG.info("Stopped playback after segment {} at playtime {}".format(
    ...
    def write(self, segment):
        ...
        config_client.LOG.info("Writing segment {} at time {}".format(segment['segment_number'],
        ...
        config_client.LOG.debug("Incrementing buffer_length by {}. dash_buffer = {}".format(
    ...
    def start(self):
        ...
        config_client.LOG.info("Starting the Player")
    ...
    def stop(self):
        ...
        self.set_state("STOP")
        self.log_entry("Stopped")
        config_client.LOG.info("Stopped the playback")
    ...
    def log_entry(self, action, bitrate=0):
        ...
        if self.buffer_log_file:
            ...
            config_client.LOG.info("BufferStats: EpochTime=%s,CurrentPlaybackTime=%s,CurrentBufferSize=%s,"
