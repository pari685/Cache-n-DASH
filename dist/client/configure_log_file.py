...
def configure_log_file(playback_type="", log_file=config_client.LOG_FILENAME):
    ...
    """
    config_client.LOG = logging.getLogger(config_client.LOG_NAME)
    config_client.LOG_LEVEL = logging.INFO
    config_client.LOG.setLevel(config_client.LOG_LEVEL)
    ...
    config_client.LOG.addHandler(handler1)
    ...
    if log_file:
        ...
        config_client.LOG.addHandler(handler2)
        print("Started logging in the log file:{}".format(log_file))


def write_json(json_data=config_client.JSON_HANDLE, json_file=config_client.JSON_LOG):
    """
